...
// ONE writeTransaction, so they capture atomically on-device and upload together — the feed
// reflects offline edits instantly. The diff/insert logic is shared (@pace/api-client) so
// web and mobile record identical entries; only the uuid factory is platform-specific.
