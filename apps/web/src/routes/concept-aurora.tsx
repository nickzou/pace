...
// Dev-only. Aurora visual style locked to the Sidebar layout, with a swappable PRIMARY
// ACCENT — the deep-indigo glass shell stays fixed; only the accent (gradient, glow,
// active states, brand, primary list) changes, so it's a pure comparison of accent hue.
export const Route = createFileRoute("/concept-aurora")({
  beforeLoad: () => {
...
// Fixed Aurora shell — everything that is NOT the accent.
const shell = {
  bg: "#0b0a14",
...
function readableText(hex: string) {
  const { r, g, b } = hexToRgb(hex)
  return 0.299 * r + 0.587 * g + 0.114 * b > 150 ? "#0b0a14" : "#ffffff"
}
...
function Aurora() {
  ...
  const soft = rgba(a.from, 0.16)
  const onAccent = readableText(a.from)
  const glow = `0 0 0 1px rgba(255,255,255,0.04), 0 10px 34px ${rgba(a.from, 0.2)}`
  ...
  return (
    <div style={{ background: t.bg, color: t.text, fontFamily: t.font }} className="flex h-screen">
            ...
            return (
              <button
                ...
                style={{
                  ...
                  boxShadow: on ? `0 0 0 2px ${t.bg}, 0 0 0 4px ${ac.from}` : "none",
                }}
      ...
      <ConceptSwitcher current="/concept-aurora" />
    </div>
  )
}
