...
export const Route = createFileRoute("/items")({ component: Items })
...
// whenever the items table changes, whether from a local write or a row synced
...
type ItemRow = { id: string; title: string; completed: number }

function ItemList() {
  const db = usePowerSync()
  const [title, setTitle] = useState("")
  const { data: items, isLoading } = useQuery<ItemRow>(
    "SELECT id, title, completed FROM items ORDER BY created_at DESC",
  )
  ...
  async function add(event: FormEvent) {
    ...
    await db.execute(
      "INSERT INTO items (id, title, description, completed, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
  ...
  const toggle = (item: ItemRow) =>
    db.execute("UPDATE items SET completed = ?, updated_at = ? WHERE id = ?", [
      item.completed ? 0 : 1,
      new Date().toISOString(),
      item.id,
    ])

  const remove = (id: string) => db.execute("DELETE FROM items WHERE id = ?", [id])

  return (
    <section>
      <h2 className="mb-3 text-sm font-semibold uppercase tracking-wider text-neutral-400">
        Items (Generic PowerSync Resource)
      </h2>
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-neutral-500">No items yet — add your first above.</p>
      ) : (
        <ul className="space-y-2">
          {items.map((item) => (
            ...
            >
              ...
              <span className={`flex-1 ${item.completed ? "text-neutral-500 line-through" : ""}`}>
                {item.title}
              </span>
              <button
                type="button"
    ...
    <main className="min-h-screen bg-neutral-950 text-neutral-100">
      <div className="mx-auto max-w-2xl px-6 py-16">
        ...
        <header className="mb-10">
          ...
          <p className="mt-2 italic text-neutral-400">offline-first sync demo</p>
